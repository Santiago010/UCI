...
void sendMessage(zmq::socket_t& p);
void readMessage(zmq::socket_t& s);
...
int main(){
    ...
    // Crear un socket de tipo PUB (publicador)
    zmq::socket_t publisher(context, zmq::socket_type::pub);

    // Crear un socket de tipo SUB (suscriptor)
    zmq::socket_t subscriber(context, zmq::socket_type::sub);

    // Enlazar el socket al canal (puerto 5555)
    publisher.bind(brokerURI);
    subscriber.connect(brokerURI);
    subscriber.set(zmq::sockopt::subscribe, "");

    thread inputThread(userInput,ref(publisher));
    thread subThread(readMessage,ref(subscriber));
...
bool getLatitudLongitude(double &latitude, double &longitude){
...
void sendMessage(zmq::socket_t& p){
    ...
    getLatitudLongitude(latitude,longitude);
    ...
    zmq::message_t zmq_message(eMessage.begin(), eMessage.end());

    // Enviar el mensaje
    p.send(zmq_message, zmq::send_flags::none);
...
void readMessage(zmq::socket_t& s){
    ...
    zmq::message_t message;
      ...
      // Recibir un mensaje
    auto result = s.recv(message, zmq::recv_flags::none);

        // Imprimir el mensaje recibido
    // std::cout<< YELLOW  << message.to_string() << std::endl;

    if(result && message.size() > 0){
        string enMessage = message.to_string();
